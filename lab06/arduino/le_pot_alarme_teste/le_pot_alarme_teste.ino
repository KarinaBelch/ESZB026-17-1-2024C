...
void loop(){                       // Le o valor no potenciometro a cada 1 segundo
  ...
  valor_volt = (valorPot*5)/1023; 
  buffer[5] = valor_volt;
  buffer[6] = ((valorPot*5)/1023 - valor_volt) *10;
...
void funcaoDadoRecebido(int x){    // funcao chamada ao receber um dado
  ...
  if( dadoRecebido == 0x02 && x == 3 ){  // se for alteracao do limite do alarme
    ...
    if( dadoRecebido == 0x10 && x == 1 ){  // se for alteracao do limite do alarme
    byte dado1 = buffer[0];     
    byte dado2 = buffer[1];      
    int valorRecebido1 =  dado1 | ( dado2 << 8 );
    if ( valorRecebido1 >= 0 && valorRecebido1 <= 1023) {
      buffer[2] = dado1;
      buffer[3] = dado2;
      limiteAlarme = valorRecebido1;
    }
  }
}
